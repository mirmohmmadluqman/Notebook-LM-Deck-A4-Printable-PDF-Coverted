...
export default function OfficeToPdfPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const selectedFile = Array.from(e.target.files).find(file =>
      ...
      if (selectedFile) {
        ...
        setError('Please select a valid Office file.');
      }
    }
  };
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files) {
      ...
      if (droppedFile) {
        ...
        setError('Please drop a valid Office file.');
      }
    }
  };
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      alert('Note: Client-side Office to PDF conversion requires heavy external WASM libraries or server-side processing which is not part of this core application. This is a mock implementation.');
    } catch (err) {
      console.error(err);
  ...
  return (
    <main className="min-h-screen p-6 sm:p-12 max-w-3xl mx-auto flex flex-col gap-8">
      <header className="flex flex-col gap-2 border-b pb-6 text-center sm:text-left">
        <h1 className="text-4xl font-bold text-gray-900">Office to PDF</h1>
          ...
          >
            ...
            {file ? (
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-gray-500">Office files (Word, Excel, PPT) are supported</p>
              </div>
            )}
          </div>
